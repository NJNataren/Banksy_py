...
"""
Title: Plot Multi-Sample Dotplot From Config
Date: 2026-05-29
Summary: Read one or more long-format dotplot summary CSVs produced by
03_export_dotplot_data_from_config.py and render a combined multi-sample dotplot.
The JSON config defines input CSVs, optional gene filtering/grouping, ordering,
...
def load_gene_file(gene_file, gene_column, gene_group_column):
    ...
    if gene_group_column:
        gene_df[gene_group_column] = gene_df[gene_group_column].fillna("unannotated").astype(str)
...
def build_cluster_order(df, cfg):
    ...
    """
    sample_order = sort_with_optional_order(df["sample"].drop_duplicates(), cfg.get("sample_order"))
    ...
    ordered_rows = []
    unique_clusters = df[["sample", "resolution", "cluster_id", "sample_cluster"]].drop_duplicates()
    for sample in sample_order:
        ...
        for resolution in resolution_order:
            ...
            if res_df.empty:
                continue
            res_df["cluster_sort"] = pd.to_numeric(res_df["cluster_id"], errors="coerce")
            res_df = res_df.sort_values(["cluster_sort", "cluster_id"], na_position="last")
...
def plot_dotplot(df, gene_order, cluster_order, gene_groups, cfg):
    ...
    fig_width = max(figure_cfg["min_width"], len(gene_order) * figure_cfg["width_per_gene"])
    fig_height = max(
